...
function SelectFeedImg({ setFiles, setPage }) {
    ...
    // handleSelectImg 을 클릭하면 fileInputRef 를 클릭하도록 하는것
    const handleSelectImg = () => {
        fileInputRef.current.click();
    }

    const handleImgFileChange = (e) => {
        setFiles(e.target.files);  // 이미지가 체인지가 된다면 files 상태에 담아줌
        setPage(2);  // 페이지 이동도 시켜준다.
    }

    return (
        <div css={S.SelectFeedImgContainer}>
            <svg css={S.ImageBox} aria-label="이미지나 동영상과 같은 미디어를 나타내는 아이콘"
            ...
            <button css={S.SelectButton} onClick={handleSelectImg}>컴퓨터에서 선택</button>
            <input css={S.FileInput} type='file' name='file' multiple={true} // multiple : 여러개 사진선택하게 해주는거
            accept='image/gif, image/jpeg, image/png, image/jpg' ref={fileInputRef} onChange={handleImgFileChange}/>
            {/* accept: 확장자 제한 */}
        </div>
    )
}
...
function ReviewFeedImg({ files }) {
    ...
    return (
        <div css={S.ReviewContainer}>
            <Carousel 
            showArrows={true} // 화살표 버튼 표시 여부
            autoPlay={false}  // 가만히 있어도 자동으로 넘길거냐
            infiniteLoop={false}  // 우측 끝으로 갔을 때 처음으로 다시 돌아갈거냐
            showThumbs={false}>
                {imgs.map((img, index) => (
...
function AddFeedModal(props) {
    ...
    const BackButton = () => {
        return(
            <div css={S.BackButton} onClick={() => {setPage(page - 1)}}>
                <FaArrowLeftLong />
            </div>
        )
    }
    ...
    useEffect(() => {
        switch(page) {
            ...
            case 2:
                ...
                break;
            case 3:
                setBodyComponent(<><ReviewFeedImg files={files} /><div><ReviewFeedImg files={files} /></div></>);
                setLeftButton(BackButton());
                break;
            default:
        }
    }, [page]); // page 번호가 바뀔때마다 상태 변화
